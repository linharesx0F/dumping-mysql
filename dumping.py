...
def backup_mysql_db():
    ...
    for dir_path in dir_mysql:
        if os.path.exists(dir_path):  # Verifica se o caminho existe
            dir_bin = dir_path  # Atribui o caminho completo do mysqldump.exe
            break  # Sai do loop assim que encontrar um caminho válido
    ...
    # Pergunta se o usuário quer fazer o dump de todo o schema ou apenas de uma tabela
    dump_option = input("Você quer fazer o dump de todo o schema (S) ou apenas de uma tabela (T)? ").strip().upper()
    if dump_option == 'S':
        # Backup de todo o schema
